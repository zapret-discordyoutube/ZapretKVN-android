...
- **Управляемый DNS.** В автоматическом режиме сначала используется DNS профиля,
  затем защищённый DoH и только после подтверждённой DNS-ошибки — системный DNS Android. DoH идёт через
  VPN.
