...
По умолчанию автоматизация выключена и lifecycle полностью совпадает с прежним:
пользователь запускает VPN вручную, а активная сессия переживает смену underlying
сети контролируемым restart. После явного включения пользователь независимо выбирает,
должен ли KVN работать в Wi-Fi, cellular, Ethernet и неизвестных Android transport.
Дополнительный bounded-список содержит не более 32 доверенных SSID; его правило можно
временно отключить без удаления списка.
...
[`Selector.outboundSelect`](https://github.com/shtorm-7/sing-box-extended/blob/ff11f007ec798136a5de258f947a4f34011a37ea/protocol/group/selector.go#L268-L291)
