...
Статистика хранится только в памяти текущей сессии. Суммарные счётчики читаются раз в секунду только пока главная находится в lifecycle `STARTED`; при уходе с экрана stream и ticker закрываются. IP запрашивается один раз после подключения; Relay HTTPS и ICMP запускаются только явной командой «Проверить оба». Последний результат каждой пробы по каждому серверу сохраняется в `noBackupFilesDir/server-latency.json` (ключ — профиль и стабильный тег сервера плюс SHA-256-отпечаток типа и адреса, сам адрес не пишется) и показывается сразу после запуска приложения с возрастом замера; записи исчезнувших профилей и серверов удаляются при пересчёте списка серверов. Постоянное уведомление показывает только состояние, а не живую скорость.
...
Исключение — «умная проверка» (настройка «Переключать при низкой скорости», по
умолчанию включена). Пока сессия подключена, настройка включена и экран горит,
существует второй `CommandStatus` client с интервалом 5 секунд; у него свой
счётчик `speedMonitorClients`, поэтому `statusClients` при скрытой главной
остаётся 0. По его отсчётам (счётчик загрузки и число соединений ядра — трекеры traffic
manager и route ConnectionManager)
детектор ищет 20 секунд подряд спроса при скорости ниже 128 КБ/с. Сам по себе он
ничего не переключает: запускается один эпизод с ограниченной загрузкой
тестового файла через туннель (3 МиБ / 6 с, тот же URL, что у ПК). libbox не
меряет скорость outbound без переключения селектора, поэтому кандидат из той же
группы (лучший по сохранённому пингу) проверяется пробным переключением тем же
selector hot-switch, что у failover, и закрепляется только если он не ниже
порога и минимум вдвое быстрее; иначе селектор возвращается. Антифлап: 5 минут
после ложной тревоги или неясного замера, 10 минут после любого переключения,
30 минут метки «медленный», 30 минут после ручного выбора сервера (failover это не затрагивает, переключение настройки сбрасывает), не больше 3 переключений в час, дедлайн эпизода
120 секунд. Экран погашен или настройка выключена — клиента нет, и VPN в простое
не имеет периодической работы. Фактическое переключение показывается сообщением
в приложении и в тексте постоянного уведомления; решения пишутся в диагностику.
...
- `CommandStatus` с интервалом 1 секунда включается только для видимой главной; второй `CommandStatus` с интервалом 5 секунд — только для «умной проверки» при включённой настройке и горящем экране; `CommandConnections` не включается никогда; bounded `CommandLog` открывается на время connect health-check и затем только при видимой диагностике;
