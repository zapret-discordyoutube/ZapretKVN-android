...
Нет отдельного updater, WorkManager, Room, `experimental.cache_file` и сетевой загрузки во время подключения. Наборы обновляются вместе с APK через Forgejo Releases. Это делает первый запуск офлайн-предсказуемым и не позволяет недоступному Forgejo заблокировать VPN.
...
- [рабочее зеркало `geoip-ru.srs`](https://git.zapret.moe/zapretdiscordyoutube/sing-geoip-upstream/raw/branch/main/geoip-ru.srs)
...
- [проверенный снимок `geoip-ru.srs`](https://git.zapret.moe/zapretdiscordyoutube/sing-geoip-upstream/raw/commit/1e1ddcb4f385b424e474051daf185ddea7137a6c/geoip-ru.srs)
