...
- ослабление проверки исполняется только с видимым предупреждением; Hysteria2
  `pinSHA256` проверяет SHA-256 всего конечного сертификата в официальном ядре и
  продолжает работать вместе с `insecure`, как в upstream-клиенте;
