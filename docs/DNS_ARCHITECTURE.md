...
Сразу за hijack-dns добавляется reject-правило на **собственную подсеть TUN** (адреса
самого TUN, напр. `172.19.0.1/30` и `fdfe:dcba:9876::1/126`):

```json
{
  "ip_cidr": ["172.19.0.1/30", "fdfe:dcba:9876::1/126"],
  "action": "reject"
}
```

Android получает адрес TUN как сетевой DNS (`addDnsServer()`), а перехватываем мы там
только порт 53. Оппортунистический Private DNS и встроенные резолверы приложений всё
равно проверяют этот же приватный адрес по DoT/DoH/mDNS на других портах (`:853` и т.п.).
Без reject такой пакет уходит дальше по route и попадает под политику «приватный IP →
`direct`» (правило `ip_is_private`): sink дозванивается по физическому линку, где адреса
TUN не существует, и приложение висит весь таймаут соединения. Хуже того, этот
`i/o timeout` от `outbound/direct` к внутреннему DNS всплывал как ложная причина отказа
туннеля (`TARGET_NETWORK_TIMEOUT`).

Правило стоит **после** hijack-dns, поэтому порт 53 к sink уже перехвачен, а любой
другой порт получает мгновенный `reject` (RST/ICMP unreachable). Для оппортунистического
Private DNS это и есть корректный сигнал «здесь DoT нет» — Android откатывается на
plaintext 53 сразу, а не по таймауту. CIDR берётся из фактического TUN-адреса профиля,
а не хардкодится. Диагностика классифицирует любое соединение к этому диапазону как
`record_only` (`LOCAL_DNS_SINK_UNREACHABLE`), чтобы инфраструктурный шум не подменял
...
перехвату не поддаётся; DoT на 853 к **внешнему** резолверу намеренно не блокируется,
потому что тот же порт использует системный strict Private DNS Android, который
приложение обязано уважать. Reject sink-подсети (см. выше) этого не нарушает: он
ограничен адресами самого TUN и не совпадает с адресом реального strict-резолвера —
downgrade strict-DNS не происходит, отбивается лишь оппортунистическая DoT-проба к
нашему внутреннему адресу.
