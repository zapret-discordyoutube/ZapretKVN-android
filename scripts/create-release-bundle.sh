...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Сервер подписки теперь выбирается прямо в интерфейсе: на карточке профиля появилась кнопка «Серверы» со списком, адресами и текущим выбором, а редактировать JSON вручную больше не нужно.' \
        '- Выбор работает и при выключенном VPN: список серверов читается из сохранённого профиля, выбор сохраняется в `selector.default` после проверки ядром и применяется при следующем подключении. При работающем VPN сохраняется прежнее горячее переключение через libbox, без пересоздания туннеля.' \
        '- Главный экран до подключения показывает выбранный сервер активного профиля и открывает тот же список одним нажатием.' \
        '- В предпросмотре импорта подписку можно сохранить одним профилем с переключением серверов или разложить по отдельному профилю на каждый сервер (до 200); частичный импорт откатывается, если ядро отклонит хотя бы один JSON.' \
        '- Имя сервера из ссылки больше не превращается в `server-ab12cd34`: теги сохраняют буквы любого алфавита, поэтому список серверов читается на русском и с флагами.' \
        '- Транзиентный сетевой отказ больше не завершает сессию: `NET-101`, `NET-102`, `DNS-101` и `DNS-105` переводят VPN в состояние «Переподключение» с причиной и номером попытки, а сервис сам ждёт пригодную сеть по событию `ConnectivityManager` и повторяет попытку с ограниченным backoff. Профиль, allowlist, политика Android, strict Private DNS и captive portal остаются терминальными.' \
        '- Перезапуск при смене сети ждёт, пока Android действительно настроит новый линк: debounce зависит от зрелости сети, а ожидание пригодной физической сети вынесено за общий deadline подключения.' \
        '- Плитка быстрых настроек показывает «Переподключение…», а её долгое нажатие открывает приложение вместо системного «О приложении».' \
        '' \
