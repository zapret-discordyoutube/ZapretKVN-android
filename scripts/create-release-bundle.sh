...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Медленный, но рабочий туннель больше не отклоняется проверкой здоровья: HTTPS-пробы стартуют со ступенчатой задержкой в 1 секунду вместо одновременного залпа, а при провале всех трёх выполняется спасательная проба с удвоенными тайм-аутами — такое подключение помечается recovered=slow_tunnel и продолжает работать.' \
        '- 0.3.0 закрепляет серию улучшений ветки 0.2.x: честные коды ошибок проверок (VPN-200/DNS-200 вместо ложного DNS-101), WireGuard-диагностика VPN-210/VPN-211 с сырыми строками ядра в технических деталях, автоматическая адаптация режима «Автоматически» к strict Private DNS и ускоренный DNS-fallback.' \
        '' \
