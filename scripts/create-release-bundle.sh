...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Раздельные профили одной подписки теперь синхронизируются как единая группа по исходному URL.' \
        '- Новые credentials и obfs-пароли обновляются без сброса индивидуальных маршрутов каждого профиля.' \
        '- Перед сохранением показываются добавленные и удалённые серверы; вручную удалённые профили не создаются повторно.' \
        '- Изменения группы применяются атомарно, а удаление подключённого сервера безопасно отключает VPN.' \
        '' \
