...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Добавлен отдельный список приложений, которым VPN полностью блокирует доступ в сеть.' \
        '- Список блокировки пуст по умолчанию и не меняет поведение существующих установок.' \
        '- Блокировка имеет приоритет над маршрутами «через VPN» и «напрямую»; одно приложение нельзя одновременно выбрать в нескольких списках.' \
        '- Для заблокированного приложения отклоняются DNS-запросы и весь остальной трафик внутри VPN.' \
        '- Исправлено возобновление локальной публикации Stable после частично загруженного draft-релиза.' \
        '' \
