...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Сбои HTTPS- и DNS-проверок здоровья внутри туннеля получили собственные коды VPN-200 и DNS-200: при неработающем туннеле больше не показывается ложное «Системный DNS не ответил» (DNS-101) от bootstrap-резолвера.' \
        '- Диагностика ошибок показывает стабильный код сбоя вместо обфусцированного R8 имени класса.' \
        '' \
