...

{
    ...
    printf '%s\n' \
        ...
        '- Исправлен импорт стандартных VLESS XHTTP URI: параметр `extra` разбирается как URL-кодированный JSON по XTLS, а не как Base64.' \
        '- XTLS XHTTP options явно преобразуются в схему закреплённого sing-box-extended; неизвестные поля завершают импорт ошибкой вместо незаметной потери настроек.' \
