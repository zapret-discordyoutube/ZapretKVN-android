...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Исправлен импорт VLESS Encryption: параметр `encryption` из VLESS URI теперь сохраняется в конфигурации и передаётся встроенному sing-box вместо принудительного `none`.' \
        '' \
