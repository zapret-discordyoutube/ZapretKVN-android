...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Временный VPN-маршрут updater теперь направляет через выбранный VPN домен `zapret.moe` и все его поддомены.' \
        '- Усилена независимая проверка опубликованных Forgejo APK без передачи production signing key в Actions.' \
        '- Сборка core и rule-set снова использует официальные upstream-источники; CI отклоняет устаревшие зеркала.' \
        '- Баннер документации хранится внутри репозитория и не зависит от внешнего вложения.' \
        '' \
