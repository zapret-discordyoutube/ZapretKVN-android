...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Brave и официальное приложение X/Twitter добавлены в рекомендации «через VPN» для новой установки.' \
        '- Рекомендации применяются только при первой настройке: обновления никогда автоматически не меняют существующий include/exclude-список пользователя.' \
        '- Удалён механизм миграций рекомендуемых приложений и связанная с ним лишняя логика хранения.' \
        '- Локальная публикация Stable стала возобновляемой: assets загружаются последовательно, проверяются по GitHub SHA-256 и публикуются только полным набором.' \
        '' \
