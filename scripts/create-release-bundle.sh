...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- WireGuard и AmneziaWG используют единое официальное ядро Amnezia v3.1.20260828; DNS, правила и единственный системный TUN остаются у sing-box.' \
        '- Сохраняются параметры AWG 3.1, включая random_trailers и disable_cookies; неподдерживаемые настройки отклоняются явно.' \
        '- Исправлено обрезание больших UDP-пакетов; воспроизводимый патч автоматически применяется при сборке и проверяется по хешам.' \
        '- Локальное закрытие TCP-соединения остаётся в исходном журнале, но больше не подменяет причину отказа удалённого сервера.' \
        '- Код разделён по движкам, сети, диагностике и платформе; убрана дублирующая ветка обычного WireGuard.' \
        '' \
        '### Проверка релиза' \
        '' \
        '- Пройдены host/unit/integration-проверки, общий WG/AWG JSON/UAPI golden-набор и проверки UDP для обеих версий sing-box; Android instrumentation APK собран.' \
