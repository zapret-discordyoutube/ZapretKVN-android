...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Режим «Автоматически» больше не падает с ошибкой при включённом strict Private DNS: цепочка кандидатов сама сужается до «DNS Android», который уважает системный DoT, и подключение продолжается с предупреждением в диагностике.' \
        '- HTTPS-проверки Cloudflare, Google и OpenDNS выполняются параллельно: первый ответ побеждает, худший случай отказа стоит один тайм-аут вместо трёх. DNS-резолв health-проб внутри туннеля укорочен с 8 до 3 секунд — подключение и DNS-fallback стали заметно быстрее.' \
        '- Вердикт VPN-210 «WireGuard: данные не возвращаются» теперь требует доказательств мёртвого туннеля из core-лога; успешный DNS-обмен через туннель отменяет вердикт (исправлено ложное срабатывание при strict Private DNS).' \
        '' \
