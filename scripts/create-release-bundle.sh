...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- При выборе другого профиля подключённый VPN автоматически заменяет текущую сессию новой; вручную выключать и включать VPN больше не нужно.' \
        '- Повторный выбор уже подключённого профиля не перезапускает VPN.' \
        '- Экран «Маршруты» явно показывает, что редактор открывает исходный JSON профиля, а общая destination-политика накладывается при запуске и не сохраняется в нём.' \
        '- Кнопка «Добавить» на экране «Маршруты» сохраняет читаемую горизонтальную подпись рядом с длинным описанием правил.' \
        '' \
