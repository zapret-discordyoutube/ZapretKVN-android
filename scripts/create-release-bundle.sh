...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- DNS через VPN теперь обрабатывается на самом узле, чтобы для сайтов применялись его DNS-правила.' \
        '- При недоступности DNS по UDP запросы через туннель переключаются на TCP.' \
        '- Автоматический режим сначала пробует DNS профиля и DNS узла; системный DNS Android остаётся последним резервом.' \
        '- В настройках режим «Защищённый через VPN» переименован в «DNS узла через VPN» и уточнён порядок переключения DNS.' \
        '' \
        '### Проверка релиза' \
        '' \
        ...
        '- Managed DNS перехватывает TCP/UDP 53, но не встроенный DoH, DoT или mDNS; DoT на TCP 853 отклоняется; FakeIP выключен.' \
