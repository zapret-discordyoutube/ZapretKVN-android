...

{
    ...
    printf '%s\n' \
        ...
        '- Добавлен фоновый контроль работоспособности туннеля: молча переставший отвечать сервер обнаруживается и вызывает переключение; для не-Hysteria протоколов выполняется только переключение и рабочий туннель не разрывается.' \
