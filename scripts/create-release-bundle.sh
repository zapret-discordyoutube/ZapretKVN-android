...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Правило трафика и пользовательские domain/IP/CIDR/rule-set теперь являются общей политикой и применяются поверх любого выбранного VPN-профиля.' \
        '- При обновлении прежний режим и редактируемые правила активного профиля мигрируют автоматически; JSON профилей, transport и credentials не переписываются.' \
        '- Общая политика заново компилируется для выбранного профиля и проходит `Libbox.CheckConfig()` до создания TUN; пустой proxy tag использует selector/outbound текущего профиля.' \
        '- На главном экране и в подключённом VPN-уведомлении добавлено явное действие «Перезапустить».' \
        '' \
