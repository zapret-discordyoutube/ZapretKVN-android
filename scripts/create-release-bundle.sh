...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Исправлена подмена причины неудачного подключения сообщением CommandLog «context canceled».' \
        '- Первоначальная ошибка HTTPS, TLS или ядра сохраняется до закрытия сессии и больше не затирается поздними callback.' \
        '- Штатное закрытие CommandLog остаётся в диагностике, но не считается аварией; неожиданная потеря канала по-прежнему закрывает VPN с исходной причиной.' \
        '' \
        '### Проверка релиза' \
        '' \
