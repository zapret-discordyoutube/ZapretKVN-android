...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Исправлен импорт VLESS/VMess/Trojan с XHTTP: приложение принимает целый объект `xhttpSettings`, включая `mode`, `host`, `path`, вложенный `extra`, `xmux` и остальные параметры, вместо локального списка отдельных полей.' \
        '- Имена параметров Xray системно переводятся из camelCase в snake_case без изменения HTTP-заголовков и типов JSON; полный результат по-прежнему проверяется встроенным sing-box-extended до сохранения профиля.' \
        '' \
