...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- Смешанные подписки теперь добавляют все поддерживаемые серверы, а неподдерживаемые URI-схемы показываются предупреждением без раскрытия адресов и credentials.' \
        '- Fragment `#…` у HTTP(S) URL подписки удаляется перед загрузкой и сохранением источника обновления.' \
        '- Updater, документация и release tooling переведены на актуальный Forgejo-репозиторий организации `zapretkvn`.' \
        '- Сборки Forgejo Actions ограничены одним Gradle worker без постоянных Gradle/Kotlin daemon, чтобы укладываться в лимит памяти runner.' \
        '' \
