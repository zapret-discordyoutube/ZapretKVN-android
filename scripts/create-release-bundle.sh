...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- XHTTP-транспорт теперь поддерживается при импорте Trojan- и VMess-ссылок наравне с VLESS, включая сочетание с Reality.' \
        '- Режим XHTTP для VMess читается из поля type в payload; заголовочный none безопасно игнорируется.' \
        '- Поле alpn из VMess payload переносится в TLS-настройки исходящего подключения.' \
        '- Импорт VLESS XHTTP снова принимает устаревшее поле extra.xmux.cMaxLifetimeMs; значение безопасно преобразуется в актуальное для ядра h_max_reusable_secs.' \
        '' \
