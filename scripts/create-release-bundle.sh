...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- VLESS обслуживает встроенный официальный Xray, Hysteria2 — официальный Hysteria; sing-box остаётся единственным владельцем маршрутизации, DNS и TUN.' \
        '- Общий журнал показывает исходные ошибки ядер, сохраняет неизвестные сообщения и включает их в диагностику с удалением секретов.' \
        '- Ошибки сертификатов и pins больше не подменяются общим VPN-200; удалены старые пересказы причин отказа.' \
        '- Исправлена передача адреса UDP-сервера между DNS и Hysteria; закрытие старой сессии прекращает принадлежащие ей сокеты.' \
        '- Наблюдение за ошибками обязательно для готовности сессии; при потере CommandLog сохраняется исходная причина отключения.' \
        '' \
        '### Проверка релиза' \
        '' \
