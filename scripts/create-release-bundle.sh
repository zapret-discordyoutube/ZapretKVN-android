...

{
    ...
    printf '%s\n' \
        ...
        '### Что изменилось' \
        '' \
        '- VLESS-ссылки и подписки с Xray flow `xtls-rprx-vision-udp443` теперь импортируются в совместимый sing-box flow `xtls-rprx-vision`.' \
        '- Неизвестные непустые значения VLESS `flow` отклоняются импортёром с понятной ошибкой до проверки ядром.' \
        '- Добавлены JVM- и Android-регрессии; итоговый профиль принят встроенным `Libbox.checkConfig()` на физическом Pixel 9 Pro XL.' \
        '' \
